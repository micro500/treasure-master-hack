...
tm_avx2_m256_8::tm_avx2_m256_8(RNG* rng_obj) : tm_avx2_m256_8(rng_obj, 0) {}

tm_avx2_m256_8::tm_avx2_m256_8(RNG* rng_obj, const uint32_t key) : tm_avx2_m256_8(rng_obj, key, key_schedule(key, key_schedule::ALL_MAPS)) {}

tm_avx2_m256_8::tm_avx2_m256_8(RNG* rng_obj, const uint32_t key, const key_schedule& schedule_entries) : TM_base(rng_obj),
...
__forceinline void tm_avx2_m256_8::initialize()
{
	if (!initialized)
	{
	...
	obj_name = "tm_avx2_m256_8";
}

void tm_avx2_m256_8::load_data(uint8_t* new_data)
{
	for (int i = 0; i < 128; i++)
	{
...
void tm_avx2_m256_8::fetch_data(uint8_t* new_data)
{
	for (int i = 0; i < 128; i++)
	{
...
__forceinline void tm_avx2_m256_8::_expand_code(uint32_t data)
{
	...
	for (int i = 0; i < 4; i++)
	{
		_mm256_store_si256((__m256i*)(working_code_data + i * 32), wide_pattern);
	}
...
__forceinline void tm_avx2_m256_8::add_alg(uint16_t* rng_seed, uint8_t* rng_start)
{
...
__forceinline void tm_avx2_m256_8::xor_alg(uint8_t* values)
{
	for (int i = 0; i < 4; i++)
	{
		__m256i cur_val = _mm256_load_si256((__m256i*)(working_code_data + i * 32));
...
__forceinline void tm_avx2_m256_8::alg_0(uint16_t* rng_seed)
{
...
__forceinline void tm_avx2_m256_8::alg_2_sub(__m256i& cur_val, __m256i& carry)
{
	__m256i part_lo = _mm256_and_si256(_mm256_srli_epi16(cur_val, 1), mask_007F);
	__m256i part_hi_bit = _mm256_and_si256(_mm256_srli_epi16(cur_val, 8), mask_0080);
	__m256i new_lo = _mm256_or_si256(part_lo, part_hi_bit);

	__m256i low_bit0 = _mm256_and_si256(cur_val, mask_0001);
	...
	__m256i new_hi = _mm256_or_si256(
		_mm256_and_si256(_mm256_slli_epi16(cur_val, 1), mask_FE00),
		carry_in);

	__m128i lo_lane = _mm256_castsi256_si128(_mm256_and_si256(cur_val, mask_0001));
	__m128i lo_shifted = _mm_slli_si128(lo_lane, 15);
	carry = _mm256_and_si256(_mm256_inserti128_si256(_mm256_setzero_si256(), lo_shifted, 1), mask_top_01);

	cur_val = _mm256_or_si256(
		_mm256_and_si256(new_lo, mask_00FF),
		_mm256_and_si256(new_hi, mask_FF00));
}

__forceinline void tm_avx2_m256_8::alg_2(uint16_t* rng_seed)
{
...
__forceinline void tm_avx2_m256_8::alg_3(uint16_t* rng_seed)
{
	uint8_t* rng_start = rng->regular_rng_values_8 + (*rng_seed) * 128;
	xor_alg(rng_start);
}

__forceinline void tm_avx2_m256_8::alg_5_sub(__m256i& cur_val, __m256i& carry)
{
	__m256i part_lo = _mm256_and_si256(_mm256_slli_epi16(cur_val, 1), mask_00FE);
	__m256i part_lo_bit = _mm256_and_si256(_mm256_srli_epi16(cur_val, 8), mask_0001);
	__m256i new_lo = _mm256_or_si256(part_lo, part_lo_bit);

	__m256i low_bit7 = _mm256_and_si256(cur_val, mask_0080);
	__m256i shifted = _mm256_slli_epi16(low_bit7, 8);
	__m256i bridge = _mm256_permute2x128_si256(shifted, shifted, 0x81);
	__m256i carry_in = _mm256_or_si256(_mm256_alignr_epi8(bridge, shifted, 2), carry);

	__m256i new_hi = _mm256_or_si256(
		_mm256_and_si256(_mm256_srli_epi16(cur_val, 1), mask_7F00),
		carry_in);

	__m128i lo_lane = _mm256_castsi256_si128(_mm256_and_si256(cur_val, mask_0080));
	__m128i lo_shifted = _mm_slli_si128(lo_lane, 15);
	carry = _mm256_and_si256(_mm256_inserti128_si256(_mm256_setzero_si256(), lo_shifted, 1), mask_top_80);
...
__forceinline void tm_avx2_m256_8::alg_5(uint16_t* rng_seed)
{
	__m256i carry = _mm256_and_si256(
...
__forceinline void tm_avx2_m256_8::alg_6(uint16_t* rng_seed)
{
...
__forceinline void tm_avx2_m256_8::alg_7()
{
	for (int i = 0; i < 4; i++)
	{
		__m256i cur_val = _mm256_load_si256((__m256i*)(working_code_data + i * 32));
...
__forceinline void tm_avx2_m256_8::_run_alg(int algorithm_id, uint16_t* rng_seed)
{
	if (algorithm_id == 0)
	{
		alg_0(rng_seed);
...
__forceinline void tm_avx2_m256_8::_run_one_map(const key_schedule::key_schedule_entry& schedule_entry)
{
...
__forceinline void tm_avx2_m256_8::_run_all_maps()
{
...
__forceinline void tm_avx2_m256_8::_decrypt_carnival_world()
{
	xor_alg(carnival_world_data);
}

__forceinline void tm_avx2_m256_8::_decrypt_other_world()
{
	xor_alg(other_world_data);
}

__forceinline void tm_avx2_m256_8::mid_sum(__m128i& sum, __m256i& working_code, __m256i& sum_mask)
{
...
__forceinline uint16_t tm_avx2_m256_8::masked_checksum(uint8_t* mask)
{
...
__forceinline uint16_t tm_avx2_m256_8::_calculate_carnival_world_checksum()
{
	return masked_checksum(carnival_world_checksum_mask);
}

__forceinline uint16_t tm_avx2_m256_8::_calculate_other_world_checksum()
{
	return masked_checksum(other_world_checksum_mask);
}

__forceinline uint16_t tm_avx2_m256_8::_fetch_carnival_world_checksum_value()
{
...
__forceinline uint16_t tm_avx2_m256_8::_fetch_other_world_checksum_value()
{
...
__forceinline void tm_avx2_m256_8::_run_bruteforce(uint32_t data, uint8_t* result_data, uint32_t* result_size)
{
	_expand_code(data);
...
void tm_avx2_m256_8::run_bruteforce_boinc(uint32_t start_data, uint32_t amount_to_run, void(*report_progress)(double), uint8_t* result_data, uint32_t result_max_size, uint32_t* result_size)
{
...
void tm_avx2_m256_8::compute_challenge_flags(uint32_t data, uint8_t& carnival_flags_out, uint8_t& other_flags_out)
{
...
void tm_avx2_m256_8::test_algorithm(int algorithm_id, uint8_t* data, uint16_t* rng_seed)
{
	load_data(data);
	_run_alg(algorithm_id, rng_seed);
	fetch_data(data);
...
void tm_avx2_m256_8::test_algorithm_n(int algorithm_id, uint8_t* data, uint16_t* rng_seed, int iterations)
{
	load_data(data);
...
void tm_avx2_m256_8::test_expansion(uint32_t data, uint8_t* result_out)
{
	_expand_code(data);
	fetch_data(result_out);
}

void tm_avx2_m256_8::test_bruteforce_data(uint32_t data, uint8_t* result_out)
{
	_expand_code(data);
	_run_all_maps();
	fetch_data(result_out);
...
bool tm_avx2_m256_8::test_bruteforce_checksum(uint32_t data, int world)
{
	_expand_code(data);
	_run_all_maps();
...
bool tm_avx2_m256_8::initialized = false;
