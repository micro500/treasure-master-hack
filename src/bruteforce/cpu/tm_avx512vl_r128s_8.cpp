...
tm_avx512vl_r128s_8::tm_avx512vl_r128s_8(RNG* rng_obj) : tm_avx512vl_r128s_8(rng_obj, 0) {}

tm_avx512vl_r128s_8::tm_avx512vl_r128s_8(RNG* rng_obj, const uint32_t key) : tm_avx512vl_r128s_8(rng_obj, key, key_schedule(key, key_schedule::ALL_MAPS)) {}

tm_avx512vl_r128s_8::tm_avx512vl_r128s_8(RNG* rng_obj, const uint32_t key, const key_schedule& schedule_entries)
	: TM_base(rng_obj),
...
__forceinline void tm_avx512vl_r128s_8::initialize()
{
	if (!initialized)
	{
	...
	obj_name = "tm_avx512vl_r128s_8";
}

__forceinline void tm_avx512vl_r128s_8::_load_from_mem(WC_ARGS_128)
{
...
__forceinline void tm_avx512vl_r128s_8::_store_to_mem(WC_ARGS_128)
{
...
__forceinline void tm_avx512vl_r128s_8::_expand_code(uint32_t data, WC_ARGS_128)
{
	...
	uint8_t* rng_start = rng->expansion_values_128_8_shuffled;
	uint16_t rng_seed = (key >> 16) & 0xFFFF;

	add_alg(WC_PASS_128, &rng_seed, rng_start);
}

void tm_avx512vl_r128s_8::load_data(uint8_t* new_data)
{
	for (int i = 0; i < 128; i++)
	{
...
void tm_avx512vl_r128s_8::fetch_data(uint8_t* new_data)
{
	for (int i = 0; i < 128; i++)
	{
...
__forceinline void tm_avx512vl_r128s_8::_run_alg(WC_ARGS_128, int algorithm_id, uint16_t* rng_seed)
{
	if (algorithm_id == 0)
	{
	...
	{
		uint8_t* rng_start = rng->regular_rng_values_128_8_shuffled;
...
__forceinline void tm_avx512vl_r128s_8::alg_0(WC_ARGS_128, uint16_t* rng_seed)
{
	uint8_t* rng_start = rng->alg0_values_128_8_shuffled + ((*rng_seed) * 128);
...
__forceinline void tm_avx512vl_r128s_8::alg_0_sub(__m128i& working_code, uint8_t* rng_start)
{
	__m128i rng_val = _mm_load_si128((__m128i*)(rng_start));
	working_code = _mm_ternarylogic_epi32(_mm_slli_epi16(working_code, 1), mask_FE, rng_val, 0xEA);
}

__forceinline void tm_avx512vl_r128s_8::alg_2_sub(__m128i& working_a, __m128i& working_b, __m128i& carry)
{
	// bytewise right shift (cross-lane carry source) and next carry, both from original working_a
	__m128i cur_val1_srl = _mm_srli_si128(working_a, 1);
	__m128i next_carry = _mm_and_si128(_mm_slli_si128(working_a, 15), mask_top_01);

	// working_a = (working_a >> 1) & mask_7F | working_b & mask_80
	// 0xD8 = (b&c)|(a&~c): selects working_b where mask_80=1, shifted_a where mask_80=0
	working_a = _mm_ternarylogic_epi32(_mm_srli_epi16(working_a, 1), working_b, mask_80, 0xD8);

	// working_b = (working_b << 1) & mask_FE | cur_val1_srl & mask_01 | carry
	// 0xE4 = (a&c)|(b&~c): selects shifted_wb where mask_FE=1, cur_val1_srl where mask_FE=0
...
__forceinline void tm_avx512vl_r128s_8::alg_2(WC_ARGS_128, uint16_t* rng_seed)
{
...
__forceinline void tm_avx512vl_r128s_8::alg_3(WC_ARGS_128, uint16_t* rng_seed)
{
	uint8_t* rng_start = rng->regular_rng_values_128_8_shuffled + ((*rng_seed) * 128);

	xor_alg(WC_PASS_128, rng_start);
}

__forceinline void tm_avx512vl_r128s_8::xor_alg(WC_ARGS_128, uint8_t* values)
{
...
__forceinline void tm_avx512vl_r128s_8::alg_5_sub(__m128i& working_a, __m128i& working_b, __m128i& carry)
{
	// bytewise right shift (cross-lane carry source) and next carry, both from original working_a
	__m128i cur_val1_srl = _mm_srli_si128(working_a, 1);
	__m128i next_carry = _mm_and_si128(_mm_slli_si128(working_a, 15), mask_top_80);

	// working_a = (working_a << 1) & mask_FE | working_b & mask_01
	// 0xD8 = (b&c)|(a&~c): selects working_b where mask_01=1, shifted_a where mask_01=0
	working_a = _mm_ternarylogic_epi32(_mm_slli_epi16(working_a, 1), working_b, mask_01, 0xD8);

	// working_b = (working_b >> 1) & mask_7F | cur_val1_srl & mask_80 | carry
	// 0xE4 = (a&c)|(b&~c): selects shifted_wb where mask_7F=1, cur_val1_srl where mask_7F=0
	working_b = _mm_or_si128(
...
__forceinline void tm_avx512vl_r128s_8::alg_5(WC_ARGS_128, uint16_t* rng_seed)
{
	__m128i carry = _mm_and_si128(
...
__forceinline void tm_avx512vl_r128s_8::alg_6(WC_ARGS_128, uint16_t* rng_seed)
{
	uint8_t* rng_start = rng->alg6_values_128_8_shuffled + ((*rng_seed) * 128);
...
__forceinline void tm_avx512vl_r128s_8::alg_6_sub(__m128i& working_code, uint8_t* rng_start)
{
	__m128i rng_val = _mm_load_si128((__m128i*)(rng_start));
	working_code = _mm_ternarylogic_epi32(_mm_srli_epi16(working_code, 1), mask_7F, rng_val, 0xEA);
}

__forceinline void tm_avx512vl_r128s_8::alg_7(WC_ARGS_128)
{
...
__forceinline void tm_avx512vl_r128s_8::add_alg(WC_ARGS_128, uint16_t* rng_seed, uint8_t* rng_start)
{
...
__forceinline bool tm_avx512vl_r128s_8::check_carnival_world_checksum(WC_ARGS_128)
{
	return _calculate_carnival_world_checksum(WC_PASS_128) == _fetch_carnival_world_checksum_value(WC_PASS_128);
}

__forceinline bool tm_avx512vl_r128s_8::check_other_world_checksum(WC_ARGS_128)
{
	return _calculate_other_world_checksum(WC_PASS_128) == _fetch_other_world_checksum_value(WC_PASS_128);
}

__forceinline void tm_avx512vl_r128s_8::mid_sum(__m128i& sum, __m128i& working_code, __m128i& sum_mask, __m128i& lo_mask)
{
...
__forceinline void tm_avx512vl_r128s_8::_run_one_map(WC_ARGS_128, const key_schedule::key_schedule_entry& schedule_entry)
{
...
__forceinline void tm_avx512vl_r128s_8::_run_all_maps(WC_ARGS_128)
{
...
__forceinline void tm_avx512vl_r128s_8::_decrypt_carnival_world(WC_ARGS_128)
{
	xor_alg(WC_PASS_128, carnival_world_data_shuffled);
}

__forceinline void tm_avx512vl_r128s_8::_decrypt_other_world(WC_ARGS_128)
{
	xor_alg(WC_PASS_128, other_world_data_shuffled);
}

__forceinline uint16_t tm_avx512vl_r128s_8::masked_checksum(WC_ARGS_128, uint8_t* mask)
{
...
__forceinline uint16_t tm_avx512vl_r128s_8::_calculate_carnival_world_checksum(WC_ARGS_128)
{
	return masked_checksum(WC_PASS_128, carnival_world_checksum_mask_shuffled);
}

__forceinline uint16_t tm_avx512vl_r128s_8::_calculate_other_world_checksum(WC_ARGS_128)
{
	return masked_checksum(WC_PASS_128, other_world_checksum_mask_shuffled);
}

__forceinline uint16_t tm_avx512vl_r128s_8::fetch_checksum_value(WC_ARGS_128, uint8_t code_length)
{
	_store_to_mem(WC_PASS_128);

	unsigned char checksum_low = (uint8_t)((uint8_t*)working_code_data)[shuffle_8((127 - code_length), 128)];
	unsigned char checksum_hi = (uint8_t)((uint8_t*)working_code_data)[shuffle_8((127 - (code_length + 1)), 128)];
	uint16_t checksum = static_cast<uint16_t>((checksum_hi << 8) | checksum_low);

	return checksum;
}

__forceinline uint16_t tm_avx512vl_r128s_8::_fetch_carnival_world_checksum_value(WC_ARGS_128)
{
	return fetch_checksum_value(WC_PASS_128, CARNIVAL_WORLD_CODE_LENGTH - 2);
}

__forceinline uint16_t tm_avx512vl_r128s_8::_fetch_other_world_checksum_value(WC_ARGS_128)
{
	return fetch_checksum_value(WC_PASS_128, OTHER_WORLD_CODE_LENGTH - 2);
}

template<bool CHECK_CHECKSUM, int WORLD>
std::optional<uint8_t> tm_avx512vl_r128s_8::_decrypt_check(WC_ARGS_128)
{
	WC_XOR_VARS_128;
	WC_COPY_XOR_128;
...
__forceinline void tm_avx512vl_r128s_8::_run_bruteforce(WC_ARGS_128, uint32_t data, uint8_t* result_data, uint32_t* result_size)
{
...
void tm_avx512vl_r128s_8::run_bruteforce_boinc(uint32_t start_data, uint32_t amount_to_run, void(*report_progress)(double), uint8_t* result_data, uint32_t result_max_size, uint32_t* result_size)
{
	WC_VARS_128;
...
void tm_avx512vl_r128s_8::compute_challenge_flags(uint32_t data, uint8_t& carnival_flags_out, uint8_t& other_flags_out)
{
	WC_VARS_128;
...
void tm_avx512vl_r128s_8::test_algorithm(int algorithm_id, uint8_t* data, uint16_t* rng_seed)
{
	WC_VARS_128;
	load_data(data);
...
void tm_avx512vl_r128s_8::test_algorithm_n(int algorithm_id, uint8_t* data, uint16_t* rng_seed, int iterations)
{
	WC_VARS_128;
	load_data(data);
	_load_from_mem(WC_PASS_128);
...
void tm_avx512vl_r128s_8::test_expansion(uint32_t data, uint8_t* result_out)
{
	WC_VARS_128;
	_expand_code(data, WC_PASS_128);
	_store_to_mem(WC_PASS_128);
...
void tm_avx512vl_r128s_8::test_bruteforce_data(uint32_t data, uint8_t* result_out)
{
	WC_VARS_128;
	_expand_code(data, WC_PASS_128);
	_run_all_maps(WC_PASS_128);
...
bool tm_avx512vl_r128s_8::test_bruteforce_checksum(uint32_t data, int world)
{
	WC_VARS_128;
	_expand_code(data, WC_PASS_128);
	_run_all_maps(WC_PASS_128);
...
bool tm_avx512vl_r128s_8::initialized = false;
