...
void tm_avx512bw_r512_map_8::generate_map_rng()
{
	...
	regular_rng_values_for_seeds = rng->generate_regular_rng_values_for_seeds_8(const_cast<uint16_t*>(schedule_entries->seeds), schedule_entries->entry_count);
	alg0_values_for_seeds = rng->generate_alg0_values_for_seeds_8(const_cast<uint16_t*>(schedule_entries->seeds), schedule_entries->entry_count);
	alg6_values_for_seeds = rng->generate_alg6_values_for_seeds_8(const_cast<uint16_t*>(schedule_entries->seeds), schedule_entries->entry_count);
}
...
__forceinline void tm_avx512bw_r512_map_8::_expand_code(uint32_t data, WC_ARGS_512)
{
	...
	uint8_t* rng_start = expansion_values_for_seed.get();
...
__forceinline void tm_avx512bw_r512_map_8::_run_one_map(WC_ARGS_512, int map_idx)
{
	...
	const uint8_t* reg_base = regular_rng_values_for_seeds.get() + map_idx * 2048;
	const uint8_t* alg0_base = alg0_values_for_seeds.get() + map_idx * 2048;
	const uint8_t* alg6_base = alg6_values_for_seeds.get() + map_idx * 2048;
...
void tm_avx512bw_r512_map_8::test_algorithm_chain(const uint8_t* algorithm_ids, int chain_length,
                                                  uint8_t* data, uint16_t* rng_seed)
{
	...
	alg0_values_for_seeds = rng->generate_alg0_values_for_seeds(rng_seed, 1, false);
	regular_rng_values_for_seeds = rng->generate_regular_rng_values_for_seeds_8(rng_seed, 1);
	alg6_values_for_seeds = rng->generate_alg6_values_for_seeds_8(rng_seed, 1);

	uint8_t* reg_base = regular_rng_values_for_seeds.get();
	uint8_t* alg0_base = alg0_values_for_seeds.get();
	uint8_t* alg6_base = alg6_values_for_seeds.get();
	uint16_t local_pos = 2047;
...
void tm_avx512bw_r512_map_8::set_key(uint32_t new_key)
{
