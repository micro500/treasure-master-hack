...
tm_avx2_r256_8::tm_avx2_r256_8(RNG* rng_obj) : tm_avx2_r256_8(rng_obj, 0) {}

tm_avx2_r256_8::tm_avx2_r256_8(RNG* rng_obj, const uint32_t key) : tm_avx2_r256_8(rng_obj, key, key_schedule(key, key_schedule::ALL_MAPS)) {}

tm_avx2_r256_8::tm_avx2_r256_8(RNG* rng_obj, const uint32_t key, const key_schedule& schedule_entries) : TM_base(rng_obj),
...
__forceinline void tm_avx2_r256_8::initialize()
{
	if (!initialized)
	{
	...
	obj_name = "tm_avx2_r256_8";
}

__forceinline void tm_avx2_r256_8::_load_from_mem(WC_ARGS_256)
{
...
__forceinline void tm_avx2_r256_8::_store_to_mem(WC_ARGS_256)
{
...
__forceinline void tm_avx2_r256_8::_expand_code(uint32_t data, WC_ARGS_256)
{
...
void tm_avx2_r256_8::load_data(uint8_t* new_data)
{
	for (int i = 0; i < 128; i++)
		working_code_data[i] = new_data[i];
}

void tm_avx2_r256_8::fetch_data(uint8_t* new_data)
{
	for (int i = 0; i < 128; i++)
		new_data[i] = working_code_data[i];
}

__forceinline void tm_avx2_r256_8::_run_alg(WC_ARGS_256, int algorithm_id, uint16_t* rng_seed)
{
	if (algorithm_id == 0)
	{
...
__forceinline void tm_avx2_r256_8::alg_0(WC_ARGS_256, uint16_t* rng_seed)
{
	uint8_t* rng_start = rng->alg0_values_8 + (*rng_seed) * 128;

	wc0 = _mm256_slli_epi16(wc0, 1);
	wc0 = _mm256_and_si256(wc0, mask_FE);
	wc0 = _mm256_or_si256(wc0, _mm256_load_si256((__m256i*)(rng_start)));

	wc1 = _mm256_slli_epi16(wc1, 1);
	wc1 = _mm256_and_si256(wc1, mask_FE);
	wc1 = _mm256_or_si256(wc1, _mm256_load_si256((__m256i*)(rng_start + 32)));

	wc2 = _mm256_slli_epi16(wc2, 1);
	wc2 = _mm256_and_si256(wc2, mask_FE);
	wc2 = _mm256_or_si256(wc2, _mm256_load_si256((__m256i*)(rng_start + 64)));

	wc3 = _mm256_slli_epi16(wc3, 1);
	wc3 = _mm256_and_si256(wc3, mask_FE);
	wc3 = _mm256_or_si256(wc3, _mm256_load_si256((__m256i*)(rng_start + 96)));
}

// Processes 32 bytes (16 byte-pairs) of a natural-layout register for alg_2.
// Each 16-bit word k holds pair (b[2k]=lo, b[2k+1]=hi).
//   new_even = (b[2k] >> 1) | (b[2k+1] & 0x80)
//   new_odd  = (b[2k+1] << 1) | carry_k
// carry_k = bit0(b[2k+2]), entering from the high end (byte 31) and exiting at byte 0.
__forceinline void tm_avx2_r256_8::alg_2_sub(__m256i& wc, __m256i& carry)
{
...
__forceinline void tm_avx2_r256_8::alg_2(WC_ARGS_256, uint16_t* rng_seed)
{
...
__forceinline void tm_avx2_r256_8::alg_3(WC_ARGS_256, uint16_t* rng_seed)
{
	uint8_t* rng_start = rng->regular_rng_values_8 + (*rng_seed) * 128;
	xor_alg(WC_PASS_256, rng_start);
}

// Symmetric to alg_2_sub for left-shift carry semantics.
//   new_even = (b[2k] << 1) | (b[2k+1] & 0x01)
//   new_odd  = (b[2k+1] >> 1) | carry_k
// carry_k = bit7(b[2k+2]), entering from byte 31 and exiting at byte 0.
__forceinline void tm_avx2_r256_8::alg_5_sub(__m256i& wc, __m256i& carry)
{
	...
	__m256i new_hi = _mm256_or_si256(
		_mm256_and_si256(_mm256_srli_epi16(wc, 1), _mm256_set1_epi16(0x7F00)),
		carry_in);
...
__forceinline void tm_avx2_r256_8::alg_5(WC_ARGS_256, uint16_t* rng_seed)
{
	__m256i carry = _mm256_and_si256(
...
__forceinline void tm_avx2_r256_8::alg_6(WC_ARGS_256, uint16_t* rng_seed)
{
...
__forceinline void tm_avx2_r256_8::alg_7(WC_ARGS_256)
{
...
__forceinline void tm_avx2_r256_8::add_alg(WC_ARGS_256, uint16_t* rng_seed, uint8_t* rng_start)
{
...
__forceinline void tm_avx2_r256_8::xor_alg(WC_ARGS_256, uint8_t* values)
{
...
void tm_avx2_r256_8::_run_one_map(WC_ARGS_256, const key_schedule::key_schedule_entry& schedule_entry)
{
	...
	for (int i = 0; i < 16; i++)
	{
		// Natural layout: bytes 0..31 are in wc0; store only wc0 to read byte i
		_mm256_store_si256((__m256i*)(working_code_data), wc0);
...
__forceinline void tm_avx2_r256_8::_run_all_maps(WC_ARGS_256)
{
...
__forceinline void tm_avx2_r256_8::_decrypt_carnival_world(WC_ARGS_256)
{
	xor_alg(WC_PASS_256, carnival_world_data);
}

__forceinline void tm_avx2_r256_8::_decrypt_other_world(WC_ARGS_256)
{
	xor_alg(WC_PASS_256, other_world_data);
}

__forceinline void tm_avx2_r256_8::mid_sum(__m128i& sum, __m256i& working_code, __m256i& sum_mask, __m128i& lo_mask)
{
...
__forceinline uint16_t tm_avx2_r256_8::masked_checksum(WC_ARGS_256, uint8_t* mask)
{
...
__forceinline uint16_t tm_avx2_r256_8::_calculate_carnival_world_checksum(WC_ARGS_256)
{
	return masked_checksum(WC_PASS_256, carnival_world_checksum_mask);
}

__forceinline uint16_t tm_avx2_r256_8::_calculate_other_world_checksum(WC_ARGS_256)
{
	return masked_checksum(WC_PASS_256, other_world_checksum_mask);
}

__forceinline uint16_t tm_avx2_r256_8::fetch_checksum_value(WC_ARGS_256, uint8_t code_length)
{
	// Store wc0 and wc1 (bytes 0..63), which cover both checksum positions
	_mm256_store_si256((__m256i*)(working_code_data), wc0);
	_mm256_store_si256((__m256i*)(working_code_data + 32), wc1);
...
__forceinline uint16_t tm_avx2_r256_8::_fetch_carnival_world_checksum_value(WC_ARGS_256)
{
	return fetch_checksum_value(WC_PASS_256, CARNIVAL_WORLD_CODE_LENGTH - 2);
}

__forceinline uint16_t tm_avx2_r256_8::_fetch_other_world_checksum_value(WC_ARGS_256)
{
	return fetch_checksum_value(WC_PASS_256, OTHER_WORLD_CODE_LENGTH - 2);
}

__forceinline bool tm_avx2_r256_8::check_carnival_world_checksum(WC_ARGS_256)
{
	return _calculate_carnival_world_checksum(WC_PASS_256) == _fetch_carnival_world_checksum_value(WC_PASS_256);
}

__forceinline bool tm_avx2_r256_8::check_other_world_checksum(WC_ARGS_256)
{
...
std::optional<uint8_t> tm_avx2_r256_8::_decrypt_check(WC_ARGS_256)
{
	WC_XOR_VARS_256;
	WC_COPY_XOR_256;
...
__forceinline void tm_avx2_r256_8::_run_bruteforce(WC_ARGS_256, uint32_t data, uint8_t* result_data, uint32_t* result_size)
{
...
void tm_avx2_r256_8::run_bruteforce_boinc(uint32_t start_data, uint32_t amount_to_run, void(*report_progress)(double), uint8_t* result_data, uint32_t result_max_size, uint32_t* result_size)
{
	WC_VARS_256;
...
void tm_avx2_r256_8::compute_challenge_flags(uint32_t data, uint8_t& carnival_flags_out, uint8_t& other_flags_out)
{
	WC_VARS_256;
...
void tm_avx2_r256_8::test_algorithm(int algorithm_id, uint8_t* data, uint16_t* rng_seed)
{
	WC_VARS_256;
	load_data(data);
...
void tm_avx2_r256_8::test_algorithm_n(int algorithm_id, uint8_t* data, uint16_t* rng_seed, int iterations)
{
	WC_VARS_256;
	load_data(data);
	_load_from_mem(WC_PASS_256);
...
void tm_avx2_r256_8::test_expansion(uint32_t data, uint8_t* result_out)
{
	WC_VARS_256;
	_expand_code(data, WC_PASS_256);
	_store_to_mem(WC_PASS_256);
...
void tm_avx2_r256_8::test_bruteforce_data(uint32_t data, uint8_t* result_out)
{
	WC_VARS_256;
	_expand_code(data, WC_PASS_256);
	_run_all_maps(WC_PASS_256);
...
bool tm_avx2_r256_8::test_bruteforce_checksum(uint32_t data, int world)
{
	WC_VARS_256;
	_expand_code(data, WC_PASS_256);
	_run_all_maps(WC_PASS_256);
...
bool tm_avx2_r256_8::initialized = false;
