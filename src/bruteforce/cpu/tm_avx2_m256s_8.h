#ifndef TM_AVX2_M256S_8_H
#define TM_AVX2_M256S_8_H
#include <optional>
#include "simd.h"
...
// Shuffled AVX2 256-bit memory-based implementation (32-bit only).
// Interleaves even and odd bytes into separate YMM registers.
// Uses shuffled RNG tables with carry values indexed per-position.
// Carry seeds: alg2_values_256_8, alg5_values_256_8 (1 carry value per position).

class tm_avx2_m256s_8 : public TM_base
{
public:
	tm_avx2_m256s_8(RNG* rng);
	tm_avx2_m256s_8(RNG* rng, uint32_t key);
	tm_avx2_m256s_8(RNG* rng, const uint32_t key, const key_schedule& schedule_entries);
