...
__forceinline void tm_64_16::initialize()
{
	if (!_initialized)
	{
		...
		auto _r5 = rng->generate_alg2_values_64_16();
		auto _r6 = rng->generate_alg4_values_16();
		auto _r7 = rng->generate_alg5_values_64_16();
		...
		_alg2_64_16 = static_cast<uint64_t*>(_r5.get());
		_alg4_16 = static_cast<uint16_t*>(_r6.get());
		_alg5_64_16 = static_cast<uint64_t*>(_r7.get());
...
__forceinline void tm_64_16::alg_0(const uint16 rng_seed)
{
	for (int i = 0; i < 32; i++)
	{
		working_code_data[i] = ((working_code_data[i] << 1) | ((uint64*)_alg0_16)[(rng_seed * 128) / 4 + i]) & 0x00FF00FF00FF00FFull;
	}
}

__forceinline void tm_64_16::alg_1(const uint16 rng_seed)
{
	add_alg((uint64*)_regular_16, rng_seed);
}

__forceinline void tm_64_16::alg_2(const uint16 rng_seed)
{
	uint64 carry = _alg2_64_16[rng_seed];
	for (int i = 31; i >= 0; i--)
	{
...
__forceinline void tm_64_16::alg_3(const uint16 rng_seed)
{
	for (int i = 0; i < 32; i++)
	{
		working_code_data[i] = working_code_data[i] ^ ((uint64*)_regular_16)[(rng_seed * 128) / 4 + i];
	}
}

__forceinline void tm_64_16::alg_4(const uint16 rng_seed)
{
	add_alg((uint64*)_alg4_16, rng_seed);
}

__forceinline void tm_64_16::alg_5(const uint16 rng_seed)
{
	uint64 carry = _alg5_64_16[rng_seed];
	for (int i = 31; i >= 0; i--)
	{
...
__forceinline void tm_64_16::alg_6(const uint16 rng_seed)
{
	for (int i = 0; i < 32; i++)
	{
		working_code_data[i] = ((working_code_data[i] >> 1) | ((uint64*)_alg6_16)[(rng_seed * 128) / 4 + i]) & 0x00FF00FF00FF00FFull;
	}
}
