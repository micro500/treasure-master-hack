...
__forceinline void tm_32_16::initialize()
{
	if (!_initialized)
	{
		...
		auto _r5 = rng->generate_alg2_values_32_16();
		auto _r6 = rng->generate_alg4_values_16();
		auto _r7 = rng->generate_alg5_values_32_16();
		...
		_alg2_32_16 = static_cast<uint32_t*>(_r5.get());
		_alg4_16 = static_cast<uint16_t*>(_r6.get());
		_alg5_32_16 = static_cast<uint32_t*>(_r7.get());
...
__forceinline void tm_32_16::alg_0(const uint16 rng_seed)
{
	for (int i = 0; i < 64; i++)
	{
		working_code_data[i] = ((working_code_data[i] << 1) | ((uint32*)_alg0_16)[(rng_seed * 128) / 2 + i]) & 0xFF00FF;
	}
}

__forceinline void tm_32_16::alg_1(const uint16 rng_seed)
{
	add_alg((uint32*)_regular_16, rng_seed);
}

__forceinline void tm_32_16::alg_2(const uint16 rng_seed)
{
	uint32 carry = _alg2_32_16[rng_seed];
	for (int i = 63; i >= 0; i--)
	{
...
__forceinline void tm_32_16::alg_3(const uint16 rng_seed)
{
	for (int i = 0; i < 64; i++)
	{
		working_code_data[i] = working_code_data[i] ^ ((uint32*)_regular_16)[(rng_seed * 128) / 2 + i];
	}
}

__forceinline void tm_32_16::alg_4(const uint16 rng_seed)
{
	add_alg((uint32*)_alg4_16, rng_seed);
}

__forceinline void tm_32_16::alg_5(const uint16 rng_seed)
{
	uint32 carry = _alg5_32_16[rng_seed];
	for (int i = 63; i >= 0; i--)
	{
...
__forceinline void tm_32_16::alg_6(const uint16 rng_seed)
{
	for (int i = 0; i < 64; i++)
	{
		working_code_data[i] = ((working_code_data[i] >> 1) | ((uint32*)_alg6_16)[(rng_seed * 128) / 2 + i]) & 0xFF00FF;
	}
}
