#ifndef TM_AVX512VL_R128_8_H
#define TM_AVX512VL_R128_8_H
#include <optional>
#include "simd.h"
...
// Like tm_avx_r128_8 but uses AVX-512VL ternarylogic (vpternlogd) to merge
// shift+mask+or into a single instruction in alg_0, alg_6, and the carry
// sub-functions.  Natural (non-shuffled) memory layout: wc0 = bytes[0..15],
// wc1 = bytes[16..31], ..., wc7 = bytes[112..127].
// Tables: expansion_values_8, alg0_values_8, regular_rng_values_8,
//         alg4_values_8, alg6_values_8 (non-shuffled, stride = seed*128).
// Carry seeds: alg2_values_8_8, alg5_values_8_8 (1 byte per seed).

class tm_avx512vl_r128_8 : public TM_base
{
public:
	tm_avx512vl_r128_8(RNG* rng);
	tm_avx512vl_r128_8(RNG* rng, uint32_t key);
	tm_avx512vl_r128_8(RNG* rng, const uint32_t key, const key_schedule& schedule_entries);
...
private:
	...
	void alg_0_sub(__m128i& working_code, uint8_t* rng_start);
	...
	void alg_6_sub(__m128i& working_code, uint8_t* rng_start);
