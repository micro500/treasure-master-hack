#ifndef TM_AVX512BWVL_R512S_8_H
#define TM_AVX512BWVL_R512S_8_H
#include <optional>
#include "simd.h"
...
class tm_avx512bwvl_r512s_8 : public TM_base
{
public:
	tm_avx512bwvl_r512s_8(RNG* rng);
	tm_avx512bwvl_r512s_8(RNG* rng, uint32_t key);
	tm_avx512bwvl_r512s_8(RNG* rng, const uint32_t key, const key_schedule& schedule_entries);
...
private:
	...
	void alg_0(WC_ARGS_512, uint16_t* rng_seed);
	void alg_2(WC_ARGS_512, uint16_t* rng_seed);
	void alg_3(WC_ARGS_512, uint16_t* rng_seed);
	void alg_5(WC_ARGS_512, uint16_t* rng_seed);
	void alg_6(WC_ARGS_512, uint16_t* rng_seed);
	void alg_7(WC_ARGS_512);
	void xor_alg(WC_ARGS_512, uint8_t* values);
	void add_alg(WC_ARGS_512, uint16_t* rng_seed, uint8_t* rng_start);
	...
	void _run_alg(WC_ARGS_512, int algorithm_id, uint16_t* rng_seed);
	void _run_one_map(WC_ARGS_512, const key_schedule::key_schedule_entry& schedule_entry);
	...
	uint8_t* _expansion_512s = nullptr;
	uint16_t* _seed_fwd_1 = nullptr;
	uint16_t* _seed_fwd_128 = nullptr;
	uint8_t* _regular_8 = nullptr;
	uint8_t* _regular_512s = nullptr;
	uint8_t* _alg0_512s = nullptr;
	uint8_t* _alg2_8_8 = nullptr;
	uint8_t* _alg4_512s = nullptr;
	uint8_t* _alg5_8_8 = nullptr;
	uint8_t* _alg6_512s = nullptr;
};
#endif // TM_AVX512BWVL_R512S_8_H
