...
tm_avx512bwvl_r512s_8::tm_avx512bwvl_r512s_8(RNG* rng_obj) : tm_avx512bwvl_r512s_8(rng_obj, 0) {}

tm_avx512bwvl_r512s_8::tm_avx512bwvl_r512s_8(RNG* rng_obj, const uint32_t key) : tm_avx512bwvl_r512s_8(rng_obj, key, key_schedule(key, key_schedule::ALL_MAPS)) {}

tm_avx512bwvl_r512s_8::tm_avx512bwvl_r512s_8(RNG* rng_obj, const uint32_t key, const key_schedule& schedule_entries)
	: TM_base(rng_obj),
...
__forceinline void tm_avx512bwvl_r512s_8::initialize()
{
	if (!initialized)
	{
	...
	obj_name = "tm_avx512bwvl_r512s_8";
}

__forceinline void tm_avx512bwvl_r512s_8::_expand_code(uint32_t data, WC_ARGS_512)
{
	...
	uint8_t* rng_start = rng->expansion_values_512_8_shuffled;
	uint16_t rng_seed = (key >> 16) & 0xFFFF;

	add_alg(WC_PASS_512, &rng_seed, rng_start);
}

void tm_avx512bwvl_r512s_8::load_data(uint8_t* new_data)
{
	for (int i = 0; i < 128; i++)
	{
...
void tm_avx512bwvl_r512s_8::fetch_data(uint8_t* new_data)
{
	for (int i = 0; i < 128; i++)
	{
...
__forceinline void tm_avx512bwvl_r512s_8::_load_from_mem(WC_ARGS_512)
{
	wc0 = _mm512_loadu_si512(working_code_data);
	wc1 = _mm512_loadu_si512(working_code_data + 64);
}

__forceinline void tm_avx512bwvl_r512s_8::_store_to_mem(WC_ARGS_512)
{
...
__forceinline void tm_avx512bwvl_r512s_8::_run_alg(WC_ARGS_512, int algorithm_id, uint16_t* rng_seed)
{
	if (algorithm_id == 0)
	{
	...
	{
		uint8_t* rng_start = rng->regular_rng_values_512_8_shuffled;
...
__forceinline void tm_avx512bwvl_r512s_8::alg_0(WC_ARGS_512, uint16_t* rng_seed)
{
	uint8_t* rng_start = rng->alg0_values_512_8_shuffled + ((*rng_seed) * 128);
...
__forceinline void tm_avx512bwvl_r512s_8::alg_2_sub(__m512i& working_a, __m512i& working_b, __m512i& carry)
{
...
__forceinline void tm_avx512bwvl_r512s_8::alg_2(WC_ARGS_512, uint16_t* rng_seed)
{
	__m512i carry = _mm512_and_si512(
		_mm512_set1_epi8(static_cast<int8_t>(rng->alg2_values_8_8[*rng_seed])),
		mask_top_01);

	alg_2_sub(WC_PASS_512, carry);
}

__forceinline void tm_avx512bwvl_r512s_8::alg_3(WC_ARGS_512, uint16_t* rng_seed)
{
	uint8_t* rng_start = rng->regular_rng_values_512_8_shuffled + ((*rng_seed) * 128);
...
__forceinline void tm_avx512bwvl_r512s_8::xor_alg(WC_ARGS_512, uint8_t* values)
{
	wc0 = _mm512_xor_si512(wc0, _mm512_load_si512(values));
	wc1 = _mm512_xor_si512(wc1, _mm512_load_si512(values + 64));
}

__forceinline void tm_avx512bwvl_r512s_8::alg_5_sub(__m512i& working_a, __m512i& working_b, __m512i& carry)
{
	// Shift bytes right 1 bit
...
__forceinline void tm_avx512bwvl_r512s_8::alg_5(WC_ARGS_512, uint16_t* rng_seed)
{
	__m512i carry = _mm512_and_si512(
		_mm512_set1_epi8(static_cast<int8_t>(rng->alg5_values_8_8[*rng_seed])),
		mask_top_80);

	alg_5_sub(WC_PASS_512, carry);
}

__forceinline void tm_avx512bwvl_r512s_8::alg_6(WC_ARGS_512, uint16_t* rng_seed)
{
	uint8_t* rng_start = rng->alg6_values_512_8_shuffled + ((*rng_seed) * 128);
...
__forceinline void tm_avx512bwvl_r512s_8::alg_7(WC_ARGS_512)
{
	wc0 = _mm512_xor_si512(wc0, mask_FF);
	wc1 = _mm512_xor_si512(wc1, mask_FF);
}

__forceinline void tm_avx512bwvl_r512s_8::add_alg(WC_ARGS_512, uint16_t* rng_seed, uint8_t* rng_start)
{
...
void tm_avx512bwvl_r512s_8::_run_one_map(WC_ARGS_512, const key_schedule::key_schedule_entry& schedule_entry)
{
...
void tm_avx512bwvl_r512s_8::_run_all_maps(WC_ARGS_512)
{
...
__forceinline uint16_t tm_avx512bwvl_r512s_8::masked_checksum(WC_ARGS_512, uint8_t* mask)
{
...
__forceinline uint16_t tm_avx512bwvl_r512s_8::_calculate_carnival_world_checksum(WC_ARGS_512)
{
	return masked_checksum(WC_PASS_512, carnival_world_checksum_mask_shuffled);
}

__forceinline uint16_t tm_avx512bwvl_r512s_8::_calculate_other_world_checksum(WC_ARGS_512)
{
	return masked_checksum(WC_PASS_512, other_world_checksum_mask_shuffled);
}

__forceinline bool tm_avx512bwvl_r512s_8::check_carnival_world_checksum(WC_ARGS_512)
{
	return _calculate_carnival_world_checksum(WC_PASS_512) == _fetch_carnival_world_checksum_value(WC_PASS_512);
}

__forceinline bool tm_avx512bwvl_r512s_8::check_other_world_checksum(WC_ARGS_512)
{
	return _calculate_other_world_checksum(WC_PASS_512) == _fetch_other_world_checksum_value(WC_PASS_512);
}

__forceinline uint16_t tm_avx512bwvl_r512s_8::_fetch_carnival_world_checksum_value(WC_ARGS_512)
{
	return fetch_checksum_value(WC_PASS_512, CARNIVAL_WORLD_CODE_LENGTH - 2);
}

__forceinline uint16_t tm_avx512bwvl_r512s_8::_fetch_other_world_checksum_value(WC_ARGS_512)
{
	return fetch_checksum_value(WC_PASS_512, OTHER_WORLD_CODE_LENGTH - 2);
}

__forceinline uint16_t tm_avx512bwvl_r512s_8::fetch_checksum_value(WC_ARGS_512, uint8_t code_length)
{
	_store_to_mem(WC_PASS_512);
...
__forceinline void tm_avx512bwvl_r512s_8::_decrypt_carnival_world(WC_ARGS_512)
{
	xor_alg(WC_PASS_512, carnival_world_data_shuffled);
}

__forceinline void tm_avx512bwvl_r512s_8::_decrypt_other_world(WC_ARGS_512)
{
	xor_alg(WC_PASS_512, other_world_data_shuffled);
}

template<bool CHECK_CHECKSUM, int WORLD>
std::optional<uint8_t> tm_avx512bwvl_r512s_8::_decrypt_check(WC_ARGS_512)
{
	WC_XOR_VARS_512;
	WC_COPY_XOR_512;
...
__forceinline void tm_avx512bwvl_r512s_8::_run_bruteforce(WC_ARGS_512, uint32_t data, uint8_t* result_data, uint32_t* result_size)
{
...
void tm_avx512bwvl_r512s_8::run_bruteforce_boinc(uint32_t start_data, uint32_t amount_to_run, void(*report_progress)(double), uint8_t* result_data, uint32_t result_max_size, uint32_t* result_size)
{
	WC_VARS_512;
...
void tm_avx512bwvl_r512s_8::compute_challenge_flags(uint32_t data, uint8_t& carnival_flags_out, uint8_t& other_flags_out)
{
	WC_VARS_512;
...
void tm_avx512bwvl_r512s_8::test_algorithm(int algorithm_id, uint8_t* data, uint16_t* rng_seed)
{
	WC_VARS_512;
	load_data(data);
...
void tm_avx512bwvl_r512s_8::test_algorithm_n(int algorithm_id, uint8_t* data, uint16_t* rng_seed, int iterations)
{
	WC_VARS_512;
	load_data(data);
	_load_from_mem(WC_PASS_512);
...
void tm_avx512bwvl_r512s_8::test_expansion(uint32_t data, uint8_t* result_out)
{
	WC_VARS_512;
	_expand_code(data, WC_PASS_512);
	_store_to_mem(WC_PASS_512);
...
void tm_avx512bwvl_r512s_8::test_bruteforce_data(uint32_t data, uint8_t* result_out)
{
	WC_VARS_512;
	_expand_code(data, WC_PASS_512);
	_run_all_maps(WC_PASS_512);
...
bool tm_avx512bwvl_r512s_8::test_bruteforce_checksum(uint32_t data, int world)
{
	WC_VARS_512;
	_expand_code(data, WC_PASS_512);
	_run_all_maps(WC_PASS_512);
...
bool tm_avx512bwvl_r512s_8::initialized = false;
