...
__forceinline void tm_sse2_m128_16::initialize()
{
	if (!_initialized)
	{
		...
		auto _r5 = rng->generate_alg2_values_128_16();
		auto _r6 = rng->generate_alg4_values_16();
		auto _r7 = rng->generate_alg5_values_128_16();
		auto _r8 = rng->generate_alg6_values_16();
		...
		_alg2_128_16 = static_cast<uint8_t*>(_r5.get());
		_alg4_16 = static_cast<uint16_t*>(_r6.get());
		_alg5_128_16 = static_cast<uint8_t*>(_r7.get());
		_alg6_16 = static_cast<uint16_t*>(_r8.get());
...
void tm_sse2_m128_16::alg_0(const uint16 rng_seed)
{
	for (int i = 0; i < 16; i++)
	{
		...
		__m128i rng_val = _mm_loadu_si128((__m128i *)(((uint8*)_alg0_16) + (rng_seed * 128 * 2) + (i * 16)));
...
{
	add_alg(((uint8*)_regular_16), rng_seed);
}

void tm_sse2_m128_16::alg_2(const uint16 rng_seed)
{
	__m128i carry = _mm_loadu_si128((__m128i *)(_alg2_128_16 + (rng_seed * 16)));
	for (int i = 15; i >= 0; i--)
	{
		__m128i cur_val = _mm_loadu_si128((__m128i *)(working_code_data + i * 16));
...
void tm_sse2_m128_16::alg_3(const uint16 rng_seed)
{
	...
	{
		__m128i cur_val = _mm_loadu_si128((__m128i *)(working_code_data + i * 16));
		__m128i rng_val = _mm_loadu_si128((__m128i *)(((uint8*)_regular_16) + (rng_seed * 128 * 2) + (i * 16)));
...
{
	add_alg(((uint8*)_alg4_16), rng_seed);
}

void tm_sse2_m128_16::alg_5(const uint16 rng_seed)
{
	__m128i carry = _mm_loadu_si128((__m128i *)(_alg5_128_16 + (rng_seed * 16)));
	for (int i = 15; i >= 0; i--)
	{
		__m128i cur_val = _mm_loadu_si128((__m128i *)(working_code_data + i * 16));
...
void tm_sse2_m128_16::alg_6(const uint16 rng_seed)
{
	...
	{
		__m128i cur_val = _mm_loadu_si128((__m128i *)(working_code_data + i * 16));
		__m128i rng_val = _mm_loadu_si128((__m128i *)(((uint8*)_alg6_16) + (rng_seed * 128 * 2) + (i * 16)));
