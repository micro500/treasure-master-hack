...
tm_64_8::tm_64_8(RNG* rng_obj) : tm_64_8(rng_obj, 0) {}

tm_64_8::tm_64_8(RNG* rng_obj, const uint32_t key) : tm_64_8(rng_obj, key, key_schedule(key, key_schedule::ALL_MAPS)) {}

tm_64_8::tm_64_8(RNG* rng_obj, const uint32_t key, const key_schedule& schedule) : TM_base(rng_obj)
{
	initialize();
	this->key = key;
...
__forceinline void tm_64_8::_expand_code(uint32_t data)
{
...
__forceinline void tm_64_8::load_data(uint8_t* src)
{
	for (int i = 0; i < 128; i++)
	{
...
__forceinline void tm_64_8::fetch_data(uint8_t* dst)
{
	for (int i = 0; i < 128; i++)
	{
...
__forceinline void tm_64_8::_run_alg(int algorithm_id, uint16_t* rng_seed)
{
	if (algorithm_id == 0)
	{
		alg_0(*rng_seed);
	...
	{
		add_alg((uint64_t*)_regular_8_hi, (uint64_t*)_regular_8_lo, *rng_seed);
		*rng_seed = _seed_fwd_128[*rng_seed];
	}
	...
	{
		add_alg((uint64_t*)_alg4_8_hi, (uint64_t*)_alg4_8_lo, *rng_seed);
		*rng_seed = _seed_fwd_128[*rng_seed];
	}
...
__forceinline void tm_64_8::alg_0(const uint16_t rng_seed)
{
	for (int i = 0; i < 16; i++)
	{
		working_code_data[i] = ((working_code_data[i] << 1) & 0xFEFEFEFEFEFEFEFEull) | ((uint64_t*)_alg0_8)[(rng_seed * 128) / 8 + i];
	}
}

__forceinline void tm_64_8::alg_2(const uint16_t rng_seed)
{
	uint64_t carry = _alg2_64_8[rng_seed];
	for (int i = 15; i >= 0; i--)
	{
		uint64_t next_carry = (working_code_data[i] & 0x0000000000000001ull) << 56;
...
__forceinline void tm_64_8::xor_alg(uint64_t* working_data, const uint8_t* values)
{
	for (int i = 0; i < 16; i++)
	{
		working_data[i] = working_data[i] ^ ((uint64_t*)values)[i];
	}
}

__forceinline void tm_64_8::alg_3(const uint16_t rng_seed)
{
	xor_alg(working_code_data, &_regular_8[rng_seed * 128]);
}

__forceinline void tm_64_8::alg_5(const uint16_t rng_seed)
{
	uint64_t carry = _alg5_64_8[rng_seed];
	for (int i = 15; i >= 0; i--)
	{
		uint64_t next_carry = (working_code_data[i] & 0x0000000000000080ull) << 56;
...
__forceinline void tm_64_8::alg_6(const uint16_t rng_seed)
{
	for (int i = 0; i < 16; i++)
	{
		working_code_data[i] = ((working_code_data[i] >> 1) & 0x7F7F7F7F7F7F7F7Full) | ((uint64_t*)_alg6_8)[(rng_seed * 128) / 8 + i];
	}
}
...
__forceinline void tm_64_8::add_alg(uint64_t* addition_values_lo, uint64_t* addition_values_hi, const uint16_t rng_seed)
{
	for (int i = 0; i < 16; i++)
	{
...
__forceinline void tm_64_8::_run_one_map(const key_schedule::key_schedule_entry& schedule_entry)
{
...
__forceinline void tm_64_8::_run_all_maps()
{
...
__forceinline void tm_64_8::_decrypt_carnival_world(uint8_t* out)
{
	xor_alg((uint64_t*)out, carnival_world_data);
}

__forceinline void tm_64_8::_decrypt_other_world(uint8_t* out)
{
	xor_alg((uint64_t*)out, other_world_data);
}

__forceinline bool tm_64_8::check_carnival_world_checksum(uint8_t* working_data)
{
	return _calculate_carnival_world_checksum(working_data) == _fetch_carnival_world_checksum_value(working_data);
}

__forceinline bool tm_64_8::check_other_world_checksum(uint8_t* working_data)
{
	return _calculate_other_world_checksum(working_data) == _fetch_other_world_checksum_value(working_data);
}

__forceinline uint16_t tm_64_8::_calculate_carnival_world_checksum(uint8_t* working_data)
{
	return masked_checksum(working_data, carnival_world_checksum_mask);
}

__forceinline uint16_t tm_64_8::_calculate_other_world_checksum(uint8_t* working_data)
{
	return masked_checksum(working_data, other_world_checksum_mask);
}

__forceinline uint16_t tm_64_8::fetch_checksum_value(uint8_t* working_data, uint8_t code_length)
{
	return static_cast<uint16_t>(working_data[reverse_offset(code_length - 1)] << 8 | working_data[reverse_offset(code_length - 2)]);
}

__forceinline uint16_t tm_64_8::_fetch_carnival_world_checksum_value(uint8_t* working_data)
{
	return fetch_checksum_value(working_data, CARNIVAL_WORLD_CODE_LENGTH);
}

__forceinline uint16_t tm_64_8::_fetch_other_world_checksum_value(uint8_t* working_data)
{
	return fetch_checksum_value(working_data, OTHER_WORLD_CODE_LENGTH);
}

__forceinline uint16_t tm_64_8::masked_checksum(uint8_t* working_data, uint8_t* mask)
{
	uint16_t sum = 0;
	for (int i = 0; i < 128; i++)
	{
...
std::optional<uint8_t> tm_64_8::_decrypt_check()
{
...
__forceinline void tm_64_8::_run_bruteforce(uint32_t data, uint8_t* result_data, uint32_t* result_size)
{
	_expand_code(data);
	_run_all_maps();
...
void tm_64_8::run_bruteforce_boinc(uint32_t start_data, uint32_t amount_to_run, void(*report_progress)(double), uint8_t* result_data, uint32_t result_max_size, uint32_t* result_size)
{
...
void tm_64_8::compute_challenge_flags(uint32_t data, uint8_t& carnival_flags_out, uint8_t& other_flags_out)
{
...
void tm_64_8::test_algorithm_chain(const uint8_t* algorithm_ids, int chain_length,
...
void tm_64_8::test_algorithm_n(int algorithm_id, uint8_t* data, uint16_t* rng_seed, int iterations)
{
	load_data(data);
...
void tm_64_8::test_expansion(uint32_t data, uint8_t* result_out)
{
	_expand_code(data);
	fetch_data(result_out);
}

void tm_64_8::test_bruteforce_data(uint32_t data, uint8_t* result_out)
{
	_expand_code(data);
	_run_all_maps();
	fetch_data(result_out);
...
bool tm_64_8::test_bruteforce_checksum(uint32_t data, int world)
{
	_expand_code(data);
	_run_all_maps();
	fetch_data(decrypted_data);
