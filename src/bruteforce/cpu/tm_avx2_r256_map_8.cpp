...
tm_avx2_r256_map_8::tm_avx2_r256_map_8(RNG* rng_obj) : tm_avx2_r256_map_8(rng_obj, 0) {}

tm_avx2_r256_map_8::tm_avx2_r256_map_8(RNG* rng_obj, const uint32_t key) : tm_avx2_r256_map_8(rng_obj, key, key_schedule(key, key_schedule::ALL_MAPS)) {}

tm_avx2_r256_map_8::tm_avx2_r256_map_8(RNG* rng_obj, const uint32_t key, const key_schedule& schedule_entries)
	: TM_base(rng_obj),
...
tm_avx2_r256_map_8::~tm_avx2_r256_map_8()
{
}
...
__forceinline void tm_avx2_r256_map_8::_load_from_mem(WC_ARGS_256)
{
...
__forceinline void tm_avx2_r256_map_8::_store_to_mem(WC_ARGS_256)
{
...
void tm_avx2_r256_map_8::load_data(uint8_t* new_data)
{
	for (int i = 0; i < 128; i++)
		working_code_data[i] = new_data[i];
}

void tm_avx2_r256_map_8::fetch_data(uint8_t* new_data)
{
	for (int i = 0; i < 128; i++)
...
// ---------------------------------------------------------------------------
// Carry-propagation helpers (natural interleaved layout, AVX2)
// ---------------------------------------------------------------------------

// alg_2_sub: right-rotate variant. carry bit is at byte 31, bit 0 (mask_top_01).
// Cross-lane byte shift uses _mm256_alignr_epi8 + _mm256_permute2x128_si256.
__forceinline void tm_avx2_r256_map_8::alg_2_sub(__m256i& wc, __m256i& carry)
{
	// new even byte = (even_byte >> 1) & 0x7F | bit7(odd_byte)
	...
	// Carry bit 0 of each even byte to hi byte of previous word, across 256-bit boundary.
	...
	// new odd byte = (odd_byte << 1) & 0xFE | carry_from_previous_even_byte
...
{
	// new even byte = (even_byte << 1) & 0xFE | bit0(odd_byte)
	...
	// Carry bit 7 of each even byte to hi byte of previous word, across 256-bit boundary.
	...
	// new odd byte = (odd_byte >> 1) & 0x7F | carry_from_previous_even_byte
	__m256i new_hi = _mm256_or_si256(
		_mm256_and_si256(_mm256_srli_epi16(wc, 1), _mm256_set1_epi16(static_cast<int16_t>(0x7F00))),
		carry_in);

	// next_carry = bit 7 of byte 0, placed at byte 31
...
__forceinline void tm_avx2_r256_map_8::alg_0(WC_ARGS_256, const uint8_t* block_start)
{
...
__forceinline void tm_avx2_r256_map_8::alg_1(WC_ARGS_256, const uint8_t* block_start)
{
...
__forceinline void tm_avx2_r256_map_8::alg_2(WC_ARGS_256, uint8_t carry_byte)
{
...
__forceinline void tm_avx2_r256_map_8::alg_3(WC_ARGS_256, const uint8_t* block_start)
{
...
__forceinline void tm_avx2_r256_map_8::alg_4(WC_ARGS_256, const uint8_t* block_start)
{
...
__forceinline void tm_avx2_r256_map_8::alg_5(WC_ARGS_256, uint8_t carry_byte)
{
...
__forceinline void tm_avx2_r256_map_8::alg_6(WC_ARGS_256, const uint8_t* block_start)
{
	__m256i rng_val = _mm256_loadu_si256((const __m256i*)(block_start));
...
__forceinline void tm_avx2_r256_map_8::alg_7(WC_ARGS_256)
{
...
__forceinline void tm_avx2_r256_map_8::xor_alg(WC_ARGS_256, uint8_t* values)
{
...
__forceinline void tm_avx2_r256_map_8::_run_alg(WC_ARGS_256, int algorithm_id, uint16_t* local_pos,
...
__forceinline void tm_avx2_r256_map_8::_run_all_maps(WC_ARGS_256)
{
...
__forceinline void tm_avx2_r256_map_8::_decrypt_carnival_world(WC_ARGS_256)
{
	xor_alg(WC_PASS_256, carnival_world_data);
}

__forceinline void tm_avx2_r256_map_8::_decrypt_other_world(WC_ARGS_256)
{
...
__forceinline void tm_avx2_r256_map_8::mid_sum(
...
__forceinline uint16_t tm_avx2_r256_map_8::masked_checksum(WC_ARGS_256, uint8_t* mask)
{
...
__forceinline uint16_t tm_avx2_r256_map_8::_calculate_carnival_world_checksum(WC_ARGS_256)
{
	return masked_checksum(WC_PASS_256, carnival_world_checksum_mask);
}

__forceinline uint16_t tm_avx2_r256_map_8::_calculate_other_world_checksum(WC_ARGS_256)
{
	return masked_checksum(WC_PASS_256, other_world_checksum_mask);
}

__forceinline uint16_t tm_avx2_r256_map_8::fetch_checksum_value(WC_ARGS_256, uint8_t code_length)
{
...
__forceinline uint16_t tm_avx2_r256_map_8::_fetch_carnival_world_checksum_value(WC_ARGS_256)
{
	return fetch_checksum_value(WC_PASS_256, CARNIVAL_WORLD_CODE_LENGTH - 2);
}

__forceinline uint16_t tm_avx2_r256_map_8::_fetch_other_world_checksum_value(WC_ARGS_256)
{
	return fetch_checksum_value(WC_PASS_256, OTHER_WORLD_CODE_LENGTH - 2);
}

__forceinline bool tm_avx2_r256_map_8::check_carnival_world_checksum(WC_ARGS_256)
{
	return _calculate_carnival_world_checksum(WC_PASS_256)
	    == _fetch_carnival_world_checksum_value(WC_PASS_256);
}

__forceinline bool tm_avx2_r256_map_8::check_other_world_checksum(WC_ARGS_256)
{
...
std::optional<uint8_t> tm_avx2_r256_map_8::_decrypt_check(WC_ARGS_256)
{
	WC_XOR_VARS_256;
	WC_COPY_XOR_256;
...
__forceinline void tm_avx2_r256_map_8::_run_bruteforce(WC_ARGS_256, uint32_t data, uint8_t* result_data, uint32_t* result_size)
{
...
void tm_avx2_r256_map_8::run_bruteforce_boinc(uint32_t start_data, uint32_t amount_to_run, void(*report_progress)(double), uint8_t* result_data, uint32_t result_max_size, uint32_t* result_size)
{
	WC_VARS_256;
...
void tm_avx2_r256_map_8::compute_challenge_flags(uint32_t data, uint8_t& carnival_flags_out, uint8_t& other_flags_out)
{
	WC_VARS_256;
...
void tm_avx2_r256_map_8::test_algorithm_chain(const uint8_t* algorithm_ids, int chain_length,
                                              uint8_t* data, uint16_t* rng_seed)
{
	...
	{
		_run_alg(WC_PASS_256, algorithm_ids[i], &local_pos,
...
void tm_avx2_r256_map_8::test_algorithm_n(int algorithm_id, uint8_t* data, uint16_t* rng_seed, int iterations)
{
	WC_VARS_256;
	load_data(data);
	_load_from_mem(WC_PASS_256);
...
void tm_avx2_r256_map_8::test_expansion(uint32_t data, uint8_t* result_out)
{
	WC_VARS_256;
	_expand_code(data, WC_PASS_256);
	_store_to_mem(WC_PASS_256);
...
void tm_avx2_r256_map_8::test_bruteforce_data(uint32_t data, uint8_t* result_out)
{
	WC_VARS_256;
	_expand_code(data, WC_PASS_256);
	_run_all_maps(WC_PASS_256);
...
bool tm_avx2_r256_map_8::test_bruteforce_checksum(uint32_t data, int world)
{
	WC_VARS_256;
	_expand_code(data, WC_PASS_256);
	_run_all_maps(WC_PASS_256);
...
void tm_avx2_r256_map_8::set_key(uint32_t new_key)
{
