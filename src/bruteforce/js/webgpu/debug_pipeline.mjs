// Pure JS simulation of the bruteforce pipeline for debugging
...
function runOneMapSeq(vals, mapIdx, mapRng, nibbleSel) {
    const newVals = [...vals];
    let localPos = 0;
    ...
    for (let i = 0; i < 16; i++) {
        ...
        // barrier - all vals visible (already in vals[])
...
console.log('After expansion (JS):');
console.log('  lane0:', '0x' + vals[0].toString(16).padStart(8,'0'));
console.log('  lane1:', '0x' + vals[1].toString(16).padStart(8,'0'));

for (let m = 0; m < entries.length; m++) {
    runOneMapSeq(vals, m, mapRng, nibbleSel);
    if (m < 3) console.log(`After map ${m}: lane0=0x${vals[0].toString(16).padStart(8,'0')}`);
}

console.log('After pipeline (JS):');
console.log('  lane0: 0x' + vals[0].toString(16).padStart(8,'0'), '(expected 0xF229E2A7)');
console.log('  lane1: 0x' + vals[1].toString(16).padStart(8,'0'), '(expected 0x9A3B914B)');
console.log('  lane31: 0x' + vals[31].toString(16).padStart(8,'0'));
