...
// wc_alg_multi: chain of N alg ids against a 128-byte input buffer using a
// single 2048-byte map_rng window built from seed_in by the host. Mirrors
// the WebGPU `tm_test_wc_alg_multi` kernel.
__attribute__((reqd_work_group_size(32, 1, 1)))
__kernel void tm_test_wc_alg_multi(
    __global const uchar* input,
    __constant uint* alg_ids, int alg_count,
    __global uchar* map_rng, __global uchar* output)
{
    uint code_index = get_local_id(0);
    uint base = code_index * 4;
    ...
    for (int i = 0; i < alg_count; i++)
        cur_val = alg_seq(cur_val, alg_ids[i], &local_pos, 0, map_rng);
    output[base + 0] =  cur_val        & 0xFFu;
    output[base + 1] = (cur_val >>  8) & 0xFFu;
    output[base + 2] = (cur_val >> 16) & 0xFFu;
    output[base + 3] = (cur_val >> 24) & 0xFFu;
