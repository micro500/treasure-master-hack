...
std::vector<tree_node*>* run_key_schedule(uint8 map, std::vector<tree_node*>::iterator key_schedule_data)
{
	...
	else
	{
		...
		std::vector<tree_node*>* alg_bits = new std::vector<tree_node*>();
		if (sched_index == 0)
		{
		...
		for (int i = 0; i < 8; i++)
		{
			std::vector<tree_node*>* alg_result = key_schedule_algorithm(map, result->begin(), i);

			alg_results->push_back(and_set2(alg_result->begin(), (*alg_flags)[i], 32));
		}

		result = or_condense2(alg_results);
	}
...
{
	tree_manager* m = (*key_schedule_data)->manager;
	std::vector<tree_node*>* result = new std::vector<tree_node*>();

	std::vector<tree_node*>* temp = xor_bits2(key_schedule_data + 8, m->get_static_byte(map)->begin(), 8);

	temp = add_bits2(temp->begin(), key_schedule_data, m->new_node_bit(0), 8);
	tree_node* carry = (*temp)[8];

	std::vector<tree_node*>* inv_sched2 = new std::vector<tree_node*>();
	for (int i = 0; i < 8; i++)
	{
		inv_sched2->push_back(m->new_node(_NOT, *(key_schedule_data + 16 + i), NULL));
	}

	temp = add_bits2(temp->begin(), inv_sched2->begin(), carry, 8);
	carry = (*temp)[8];

	temp = add_bits2(temp->begin(), key_schedule_data + 24, carry, 8);
	carry = (*temp)[8];
	std::vector<tree_node*>* sched3 = new std::vector<tree_node*>();
	sched3->insert(sched3->end(), temp->begin(), temp->begin() + 8);

	temp = add_bits2(temp->begin(), key_schedule_data + 16, carry, 8);
	carry = (*temp)[8];
	std::vector<tree_node*>* sched2 = new std::vector<tree_node*>();
	sched2->insert(sched2->end(), temp->begin(), temp->begin() + 8);

	temp = add_bits2(temp->begin(), key_schedule_data + 8, carry, 8);
	carry = (*temp)[8];
	std::vector<tree_node*>* sched1 = new std::vector<tree_node*>();
	sched1->insert(sched1->end(), temp->begin(), temp->begin() + 8);

	temp = add_bits2(temp->begin(), key_schedule_data, carry, 8);
	carry = (*temp)[8];
	std::vector<tree_node*>* sched0 = new std::vector<tree_node*>();
...
std::vector<tree_node*>* key_schedule_algorithm_1(uint8 map, std::vector<tree_node*>::iterator key_schedule_data)
{
	...
	tree_node* carry = m->new_node_bit(1);

	std::vector<tree_node*>* rolling_sum = m->get_static_byte(map);

	rolling_sum = add_bits2(rolling_sum->begin(), key_schedule_data + 24, carry, 8);
	carry = (*rolling_sum)[8];
	std::vector<tree_node*>* sched3 = new std::vector<tree_node*>();
	sched3->insert(sched3->end(), rolling_sum->begin(), rolling_sum->begin() + 8);

	rolling_sum = add_bits2(rolling_sum->begin(), key_schedule_data + 16, carry, 8);
	carry = (*rolling_sum)[8];
	std::vector<tree_node*>* sched2 = new std::vector<tree_node*>();
	sched2->insert(sched2->end(), rolling_sum->begin(), rolling_sum->begin() + 8);

	rolling_sum = add_bits2(rolling_sum->begin(), key_schedule_data + 8, carry, 8);
	carry = (*rolling_sum)[8];
	std::vector<tree_node*>* sched1 = new std::vector<tree_node*>();
	sched1->insert(sched1->end(), rolling_sum->begin(), rolling_sum->begin() + 8);

	rolling_sum = add_bits2(rolling_sum->begin(), key_schedule_data, carry, 8);
	carry = (*rolling_sum)[8];
	std::vector<tree_node*>* sched0 = new std::vector<tree_node*>();
...
std::vector<tree_node*>* key_schedule_algorithm_2(uint8 map, std::vector<tree_node*>::iterator key_schedule_data)
{
	...
	std::vector<tree_node*>* temp = add_bits2(key_schedule_data, m->get_static_byte(map)->begin(), m->new_node_bit(0), 8);
...
std::vector<tree_node*>* key_schedule_algorithm_5(uint8 map, std::vector<tree_node*>::iterator key_schedule_data)
{
	...
	std::vector<tree_node*>* map_nodes = m->get_static_byte(map);

	std::vector<tree_node*>* temp = new std::vector<tree_node*>();
	temp->push_back(m->new_node_bit(0));
	temp->insert(temp->end(), map_nodes->begin(), map_nodes->begin() + 7);

	temp = xor_bits2(temp->begin(), m->get_static_byte(0xFF)->begin(), 8);
	temp = add_bits2(temp->begin(), key_schedule_data, m->new_node_bit(0), 8);

	uint8 neg_map = ((map ^ 0xFF) + 1) & 0xFF;

	temp = add_bits2(temp->begin(), m->get_static_byte(neg_map)->begin(), m->new_node_bit(0), 8);
