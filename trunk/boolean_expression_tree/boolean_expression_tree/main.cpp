...
void add_byte(int pos,unsigned char val,node * bit_field[])
{
	...
	if (pos == 0x7E)
	{
		...
		printf("%s\n",bit3->get_string().c_str());
		printf("%s\n",bit4->get_string().c_str());
		printf("%s\n",bit5->get_string().c_str());
		printf("%s\n",bit6->get_string().c_str());
		printf("%s\n",bit7->get_string().c_str());
	}*/
	
	//printf("%s\n",bit0->get_string().c_str());
	//printf("%s %s\n",bit0->left->get_string().c_str(),bit0->right->get_string().c_str());
	//printf("%08X\n",bit0);
	//bit0 = bit0->simplify();
	//printf("%s\n",bit0->get_string().c_str());
	...
	*/
	...
	for (int i = 0; i < 16; i++)
	{
		...
		if (bit2->type != VAL || bit1->type != VAL || bit0->type != VAL)
		{
			...
			std::vector<std::vector<unsigned int>> implicants;

			implicants.push_back(std::vector<unsigned int>());
			implicants.push_back(std::vector<unsigned int>());
			implicants.push_back(std::vector<unsigned int>());
			implicants.push_back(std::vector<unsigned int>());
			implicants.push_back(std::vector<unsigned int>());
		...
		if (i == 4)
		{
			...
			{
				all_bits[j]->assign_var('B',false);
				all_bits[j]->assign_var('w',false);
				all_bits[j]->assign_var('v',false);
				all_bits[j]->assign_var('x',false);
				all_bits[j]->assign_var('y',false);
				all_bits[j]->assign_var('z',false);
				all_bits[j]->assign_var('A',false);
				all_bits[j] = all_bits[j]->simplify();
			}
		}
		else if (i == 5)
		{
			for (int j = 0; j < 128 * 8; j++)
			{
				all_bits[j]->assign_var('R',false);
				all_bits[j]->assign_var('Q',false);
				all_bits[j]->assign_var('P',false);
				all_bits[j]->assign_var('O',false);
				all_bits[j]->assign_var('N',false);
				all_bits[j]->assign_var('M',false);
				all_bits[j]->assign_var('L',false);
				all_bits[j]->assign_var('J',false);
				all_bits[j]->assign_var('I',false);
				all_bits[j]->assign_var('H',false);
				all_bits[j]->assign_var('G',false);
				all_bits[j]->assign_var('F',false);
				all_bits[j]->assign_var('E',false);
				all_bits[j] = all_bits[j]->simplify();
			}
		}
		else if (i == 12)
		{
			for (int j = 0; j < 128 * 8; j++)
			{
				all_bits[j]->assign_var('C',false);
				all_bits[j]->assign_var('D',false);
				all_bits[j] = all_bits[j]->simplify();
			}
		}
		else if (i == 14)
		{
			for (int j = 0; j < 128 * 8; j++)
			{
				all_bits[j]->assign_var('K',false);
				all_bits[j]->assign_var('V',false);
				all_bits[j]->assign_var('U',false);
				all_bits[j]->assign_var('T',false);
				all_bits[j] = all_bits[j]->simplify();
			}
		}
		else if (i == 15)
		...
		if (bit2->type != VAL || bit1->type != VAL || bit0->type != VAL)
		{
			printf("Evaluation not completed\n");
			return 1;
		}
		...
		if (bit2->type == VAL && bit1->type == VAL && bit0->type == VAL)
		{
			...
			printf("Bits are constants: %02x\n",result);
		}

		/*
		if (all_bits[i*8+7]->type == VAL && all_bits[i*8+6]->type == VAL && all_bits[i*8+5]->type == VAL)
			{
				result = ((all_bits[i*8+7]->val ? 1 : 0) << 2) | ((all_bits[i*8+6]->val ? 1 : 0) << 1) | (all_bits[i*8+5]->val ? 1 : 0);
				printf("Bits are constants: %02x\n",result);
			}
			else
			{
				printf("Need evaluation:\n");
				printf("%s\n%s\n%s\n",all_bits[i*8+7]->get_string().c_str(),all_bits[i*8+6]->get_string().c_str(),all_bits[i*8+5]->get_string().c_str());
			...
			if (all_bits[i*8+3]->type == VAL && all_bits[i*8+2]->type == VAL && all_bits[i*8+1]->type == VAL)
			{
				result = ((all_bits[i*8+3]->val ? 1 : 0) << 2) | ((all_bits[i*8+2]->val ? 1 : 0) << 1) | (all_bits[i*8+1]->val ? 1 : 0);
				printf("Bits are constants: %02x\n",result);
			}
			else
			{
				printf("Need evaluation:\n");
				printf("%s\n%s\n%s\n",all_bits[i*8+3]->get_string().c_str(),all_bits[i*8+2]->get_string().c_str(),all_bits[i*8+1]->get_string().c_str());
	...
	/*
	// From the key schedule for key = 0, seed rng to 0xFF, 0xFF
	seed_rng(0xFF,0xFF);
	alg0();
