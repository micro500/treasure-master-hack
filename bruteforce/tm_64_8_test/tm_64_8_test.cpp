...
tm_64_8_test::tm_64_8_test()
{
	...
	alg2_values = new uint64[0x10000];
	generate_alg2_values_64_8(alg2_values, rng_table);

	alg5_values = new uint64[0x10000];
...
void tm_64_8_test::process_test_case(uint8 * test_case, uint16 * rng_seed, int algorithm)
{
	...
	if (algorithm == 0)
	{
		alg0((uint64*)working_code, (uint64*)alg0_values, *rng_seed);
		*rng_seed = rng_seed_forward_128[*rng_seed];
	}
	else if (algorithm == 1)
	{
		alg1((uint64*)working_code, (uint64*)regular_rng_values_lo, (uint64*)regular_rng_values_hi, *rng_seed);
		*rng_seed = rng_seed_forward_128[*rng_seed];
	}
	else if (algorithm == 2)
	{
		alg2((uint64*)working_code, (uint64*)alg2_values, *rng_seed);
		*rng_seed = rng_seed_forward_1[*rng_seed];
	}
	else if (algorithm == 3)
	{
		alg3((uint64*)working_code, (uint64*)regular_rng_values, *rng_seed);
		*rng_seed = rng_seed_forward_128[*rng_seed];
	}
	else if (algorithm == 4)
	{
		alg1((uint64*)working_code, (uint64*)alg4_values_lo, (uint64*)alg4_values_hi, *rng_seed);
		*rng_seed = rng_seed_forward_128[*rng_seed];
	}
	else if (algorithm == 5)
	{
		alg5((uint64*)working_code, (uint64*)alg5_values, *rng_seed);
		*rng_seed = rng_seed_forward_1[*rng_seed];
	}
	else if (algorithm == 6)
	{
		alg6((uint64*)working_code, (uint64*)alg6_values, *rng_seed);
		*rng_seed = rng_seed_forward_128[*rng_seed];
	}
	else if (algorithm == 7)
	{
...
void tm_64_8_test::run_iterations(uint8 * test_case, uint16 * rng_seed, int algorithm, int iterations)
{
